                ...
                {(this.state.loader == false) ?
                    <>
                                                                            ...
                                                                            <div className="container p-0">
                                                                                <Row className="w-100 no-gutters">
                                                                                    ...
                                                                                    <div className="col-12 col-md-4">
                                                                                        <img className="img-fluid rounded-bottom rounded-top" src={headerimg2} alt="..." />
                                                                                    </div>
                                                                                </Row>
                                                                            </div>
                                                                        </DropdownMenu>
                                                            ...
                                                            {(this.GetCartItems() != null && this.GetCartItems().length > 0) ?
                                                                <>
                                                                    ...
                                                                    <div className="ml-4 d-none d-md-block"> <small className="d-block text-muted">My Cart</small>
                                                                        <span className="text-dark">{this.GetCartItems().length} Items - Kes.{this.GetCartItems().reduce((fr, CartItem) => fr + (CartItem.Qty * CartItem.Rate), 0).toLocaleString(navigator.language, { minimumFractionDigits: 0 })}</span>
                                                                    </div>
                                                                </>
                                                                :
                        ...
                        <Modal isOpen={this.state.cartview} toggle={this.cartview} className="cart-modal">
                            <ModalHeader toggle={this.cartview}>Your Cart</ModalHeader>
                            <ModalBody>
                                {(this.GetCartItems() != null && this.GetCartItems().length > 0) ?
                                    <>
                                                    ...
                                                    <div className="row align-items-center">
                                                        <div className="col-5 d-flex align-items-center">
                                                            ...
                                                            <a href="product-left-image.html">
                                                                <img className="img-fluid" src={CartItem.ProductImage} alt="..." />
                                                            </a>
                                                        </div>
                                                        <div className="col-7">
                                                            {/* Title */}
                                                            <h6><a className="link-title" href="product-left-image.html">{CartItem.ProductName}</a></h6>
                                                            <div className="product-meta"><span className="mr-2 text-primary">Kes {(CartItem.Rate * CartItem.Qty).toLocaleString(navigator.language, { minimumFractionDigits: 0 })}</span><span className="text-muted">x {CartItem.Qty}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="d-flex justify-content-between align-items-center mb-8"> <span className="text-muted">Subtotal:</span>  <span className="text-dark">Kes{this.GetCartItems().reduce((fr, CartItem) => fr + (CartItem.Qty * CartItem.Rate), 0).toLocaleString(navigator.language, { minimumFractionDigits: 0 })}</span>
